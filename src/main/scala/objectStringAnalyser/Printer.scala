...
object Printer {
  def main(args: Array[String]) {
    ...
    Tree.prettyPrintNodes(parseNode(s.toCharArray.toVector, 0 ,StringBuilder.newBuilder, List.empty), 0)
  }

  @tailrec
  def unwrap(s: Vector[Char], value: StringBuilder): (String, Vector[Char]) = {
    s match {
 ...
 // @tailrec
  def parseNode(string: Vector[Char], unclosedBracket: Int, builder: StringBuilder, result: List[Tree]): List[Tree] = {
    ...
    string match {
      ...
      case IndexedSeq() => parseNode(Vector.empty, 0, StringBuilder.newBuilder, result :+ buildTreeNode())
